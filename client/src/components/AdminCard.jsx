...
const AdminCard = ({
  ...
  const nft_contract = new ethers.Contract(nft_contract_address,nftABI,signer);
      ...
      <div className="bg-gray-900 p-4 rounded-lg shadow-lg text-white w-[22rem]">
        ...
        <div className="flex space-x-2 mb-3">
          ...
          <div className='bg-[#1B2532] bg-opacity-91 p-2 rounded-lg w-full'>
            ...
            <p className="text-xs">Buyer:<br/> {Buyer}</p>
          </div>
        </div>
        <p className="text-xs mb-3">{Location}</p>
        <button className="bg-gray-200 text-black py-2 px-4 rounded-full w-full text-sm" onClick={handleClick}> Transfer NFT </button>
      </div>
    </div>
  );
};
